    ...
    const customStaticRanges = createStaticRanges([

        {
            ...
            range: () => ({
                ...
                endDate: moment().startOf('year').add(6, 'months').subtract(1, 'days').toDate(),
            }),
        },
        {
    ...
    return (
        <div className="relative" ref={pickerRef}>
            <input
                type="text"
                readOnly
                value={`${moment(state[0].startDate).format('MMM. DD, YYYY')} - ${moment(state[0].endDate).format('MMM. D, YYYY')}`}
            ...
            {showCalendar && (
                <div className="absolute z-10 mt-2 text-sm border rounded-md shadow-lg right-0 dark:text-slate-700">
                    <DateRangePicker
                        ...
                        staticRanges={[...customStaticRanges, ...defaultStaticRanges]}
                    />
                </div>
            )}
        </div>
